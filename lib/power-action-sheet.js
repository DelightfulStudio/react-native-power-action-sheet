...
import { Animated, LayoutAnimation, Modal, ScrollView, Text, View } from "react-native";
...
class PowerActionSheet extends Component {
    ...
    close = async result => {
        ...
        Animated.timing( this.state.overlayOpacity, {
            ...
            await this._promiseState( { visible: false } );
    ...
    _replace( options ) {
        LayoutAnimation.configureNext( easeInEaseOut, () =>
            this._promiseSheet( { onClose: this.state.options.onClose, ...options } )
        );
    ...
    _renderSheet( styles ) {
        ...
        switch ( stage ) {
            ...
            case Stage.opening:
            case Stage.closed:
                sheetStyle.push( { top: windowHeight, height: maxHeight * 2 } );
                break;
        }
